    ...
    public class AcademicResearchPaperAgent
    {
        ...
        public async Task<string> ResearchTopicAndSynthesizeAsync(string topic, int maxPapers = 3)
        {
            try
            {
                ...
                // Search for papers
                var searchResults = await SearchAcademicPapersAsync(topic, maxPapers);
                ...
                foreach (var url in urls.Take(maxPapers))
                {
                    try
                    {
                        var analysis = await AnalyzePaperAndGenerateBlueprintAsync(url, topic);
                ...
                if (analyses.Any())
                {
                    var synthesis = await SynthesizeMultiplePapersAsync(topic, analyses);
...
Provide specific, actionable guidance for each section.
";
...
Format it as readable code blocks.
";
...
Create a coherent, actionable synthesis that leverages insights from all analyzed papers.
";
