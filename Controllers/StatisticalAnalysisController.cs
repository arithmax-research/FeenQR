    ...
    public class StatisticalAnalysisController : ControllerBase
    {
            ...
            [FromQuery] int days = 252)
        {
            try
            {
                ...
                var volatility = CalculateVolatilityMetrics(returns);
                var distribution = AnalyzeDistribution(returns);
                ...
                return Ok(new
                {
                    ...
                    tradingCharacteristics = new
                    {
                        ...
                        highVolatility = volatility.annualized > 0.3,
                        normalReturns = Math.Abs(distribution.kurtosis - 3) < 1
                    },
                    recommendation = GenerateTradingRecommendation(returns, acf, adfTest, volatility)
                });
            }
            catch (Exception ex)
            {
