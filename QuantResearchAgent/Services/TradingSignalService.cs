...
public class TradingSignalService
{
    ...
    private Dictionary<string, double> CalculateTechnicalIndicators(IEnumerable<MarketData> historicalData)
    {
        ...
        try
        {
            ...
            // Calculate RSI
            var rsiResults = quotes.GetRsi(14).LastOrDefault();
            if (rsiResults != null)
                indicators["RSI"] = (double)(rsiResults.Rsi ?? 50);

            // Calculate MACD
            var macdResults = quotes.GetMacd(12, 26, 9).LastOrDefault();
            if (macdResults != null)
            {
            ...
            // Calculate SMA
            var sma20Results = quotes.GetSma(20).LastOrDefault();
            if (sma20Results != null)
                indicators["SMA_20"] = (double)(sma20Results.Sma ?? 0);

            var sma50Results = quotes.GetSma(50).LastOrDefault();
            if (sma50Results != null)
                indicators["SMA_50"] = (double)(sma50Results.Sma ?? 0);

            // Calculate Bollinger Bands
            var bbResults = quotes.GetBollingerBands(20, 2).LastOrDefault();
            if (bbResults != null)
            {
            ...
            var atrResults = quotes.GetAtr(14).LastOrDefault();
            if (atrResults != null)
